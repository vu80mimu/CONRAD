...
{
float wy = (float) ((y - recoSize[1] / 2.0 + 0.5f) * spacing[1]);

final PointND reconstructionPointWorld = new PointND(wx, wy, 0.d);

//intersect the projection ray with the detector
final StraightLine projectLine = new StraightLine(source, reconstructionPointWorld);
final PointND detPixel = projectLine.intersect(detLine);
...
float phi = (float) ((Math.PI / 2.0) + Math.atan2(reconstructionPointWorld.get(1), reconstructionPointWorld.get(0)));
...
for(int i=0; i<detectorPixels; ++i){
	//TODO
	double s = ((i + 0.5) * detectorSpacing)-detectorLength/2;//center pixels and considering also the mid point
	cosineKernel.setAtIndex(i, (float) (focalLength/Math.sqrt(focalLength*focalLength + s*s))); // (D/sqrt(D2 + i2))i
}
...
float val = (float) Math.pow(Math.sin((Math.PI/4.f)* (beta/(delta - gamma))),2); //sin2(pi/4 *(beta/delta-gamma)
if (Double.isNaN(val)){
continue;
}
...
float val = (float) Math.pow(Math.sin( ( (Math.PI * (Math.PI+2.d*delta-beta)) / (4.d * (delta+gamma) ))) , 2);//sin2( pi/4 pi+2delta-beta/delta+gammma
if (Double.isNaN(val)){
continue;
}
parker.setAtIndex(t, b , val);
...
String filename = "/proj/i5dmip/vu80mimu/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/Sinogram0.tif";
